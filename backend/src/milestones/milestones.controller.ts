    ...
    try {
      ...
      try {
        ...
        if (compressedBuffer.length > MAX_FILE_SIZE) {
          console.warn(`Image still too large (${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB), compressing more aggressively...`);
          ...
          if (compressedBuffer.length > MAX_FILE_SIZE) {
            console.warn(`Image still too large (${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB), using maximum compression...`);
        ...
        if (compressedBuffer.length > MAX_FILE_SIZE) {
          throw new HttpException(
            `Image is too large even after compression (${(compressedBuffer.length / 1024 / 1024).toFixed(2)}MB). Please use a smaller image.`,
            HttpStatus.BAD_REQUEST
          );
        }
        ...
        if (buffer.length > MAX_FILE_SIZE) {
          throw new HttpException(
            `Image is too large (${(buffer.length / 1024 / 1024).toFixed(2)}MB) and compression failed. Please use a smaller image.`,
            HttpStatus.BAD_REQUEST
          );
        }
      ...
      const fileName = `milestone-photos/${timestamp}-${randomId}.${extension}`;
      const thumbnailFileName = `milestone-photos/${timestamp}-${randomId}_thumb.${extension}`;
      ...
      if (thumbnailBuffer) {
        ...
        if (!thumbError && thumbData) {
          const {
            data: { publicUrl },
          } = supabase.storage.from("milestone-photos").getPublicUrl(thumbnailFileName);
