...
export class DatesController {
  ...
  async uploadImage(@Body() body: { image: string }) {
    try {
      ...
      const fileName = `date-photos/${timestamp}-${randomId}.${extension}`;
      const thumbnailFileName = `date-photos/${timestamp}-${randomId}_thumb.${extension}`;
      ...
      const bucketExists = buckets?.some(
        (bucket) => bucket.name === "date-photos"
      );
      ...
      if (!bucketExists) {
        ...
        if (createError) {
          ...
          throw new HttpException(
            `Failed to create storage bucket: ${createError.message}. Please create 'date-photos' bucket in Supabase Storage manually.`,
            HttpStatus.INTERNAL_SERVER_ERROR
          );
        }
      }
